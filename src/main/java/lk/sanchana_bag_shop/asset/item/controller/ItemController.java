  ...
  public String persist(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
    ...
    if ( item.getId() == null ) {
      ...
      if ( itemService.lastItem() == null ) {
        ...
        item.setCode("SBSI" + makeAutoGenerateNumberService.numberAutoGen(null).toString());
        item.setItemStatus(ItemStatus.JUST);
      } else {
        ...
        item.setCode("SBSI" + makeAutoGenerateNumberService.numberAutoGen(previousCode).toString());
      }
    }
